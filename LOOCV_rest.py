...
def plot_result(true, pred, corr, rmse, acc, sheet_name, save_dir):
    ...
    plt.title(f"Flow → ICP (LOOCV) — Sheet: {sheet_name}\n"
...
for i in range(1, 51):  # 1~50
    ...
    scaler_y = RobustScaler(); y_s = scaler_y.fit_transform(y)
    all_true, all_pred = [], []
    ...
    for i in range(1, len(pred)):
        if pred[i - 1] - pred[i] > max_drop:
            pred[i] = pred[i - 1] - max_drop
...
# ==== Merge all images (Vertical Grid 5×10) ====
